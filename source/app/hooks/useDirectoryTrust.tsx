 ...
 */
export function useDirectoryTrust(directory: string = process.cwd()): UseDirectoryTrustReturn {
	...
	useEffect(() => {
		const checkTrustStatus = async () => {
			try {
				...
				const isTrustedDir = trustedDirectories.some(trustedDir => 
					trustedDir === normalizedDirectory
				);
	...
	const handleConfirmTrust = useCallback(() => {
		try {
			...
			if (!trustedDirectories.includes(directory)) {
				trustedDirectories.push(directory);
				preferences.trustedDirectories = trustedDirectories;
				savePreferences(preferences);
				
				if (shouldLog('info')) {
					console.log(`Directory added to trusted list: ${directory}`);
				}
			}
