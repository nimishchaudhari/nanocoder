...
import {checkPerformanceMonitoring} from './performance-check.js';
...
test('checkPerformanceMonitoring returns health check result', t => {
	const config = createMockConfig();
	const result = checkPerformanceMonitoring(config);

	t.is(result.name, 'performance-monitoring');
