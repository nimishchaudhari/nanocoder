...
export class CustomCommandExecutor {
  constructor(private chatSession: ChatSession) {}
  
  /**
   * Execute a custom command with given arguments
   */
  async execute(command: CustomCommand, args: string[]): Promise<void> {
    ...
    const promptContent = substituteTemplateVariables(command.content, variables);
    ...
    await this.chatSession.processUserInput(fullPrompt);
  }
