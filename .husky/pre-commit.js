...
function setupPath() {
  ...
  if (nodeVersion) {
    ...
    if (homeDir) {
      if (platform === 'win32') {
        ...
        const nvmNodePath = join(homeDir, '.nvm', 'versions', 'node', nodeVersion, 'bin');
        if (!additionalPaths.includes(nvmNodePath) && fs.existsSync(nvmNodePath)) {
  ...
  const newPath = [...validPaths, currentPath.split(delimiter)].flat().join(delimiter);
...
function findPnpm() {
  ...
  try {
    ...
    const pnpmPaths = result.trim().split('\n').filter(path => path && path.trim());
    ...
    for (const pathDir of pathArray) {
      ...
      const pnpmPath = join(pathDir, platform === 'win32' ? 'pnpm.cmd' : 'pnpm');
...
function runLintStaged() {
  return new Promise((resolve, reject) => {
    ...
    if (!pnpmPath) {
      ...
      console.error('Please ensure pnpm is installed and available in your PATH');
