...
import { ExistingRulesExtractor, type ExistingRules } from './existing-rules-extractor.js';

export class AgentsTemplateGenerator {
  /**
   * Generate AGENTS.md content based on project analysis
   */
  public static generateAgentsMd(analysis: ProjectAnalysis, existingRules?: ExistingRules[]): string {
  ...
  private static getFileDescription(file: string): string {
    ...
    if (fileName.includes('package.json')) return 'Node.js dependencies and scripts';
