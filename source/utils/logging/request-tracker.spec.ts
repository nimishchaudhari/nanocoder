...
import {RequestTracker, RequestMetadata, RequestStats, globalRequestTracker} from './request-tracker.js';
import {trackRequest, httpTracker, aiTracker, mcpTracker} from './request-tracker.js';

// Helper function to create test request metadata
function createTestRequestMetadata(overrides: Partial<RequestMetadata> = {}): Omit<RequestMetadata, 'id' | 'startTime' | 'status' | 'correlationId'> {
    return {
        type: 'http',
        method: 'GET',
        url: '/test',
...
test('RequestTracker getStats calculates correct statistics', t => {
    ...
    // Add failed request
    const failRequestId = tracker.startRequest(createTestRequestMetadata({
        type: 'http',
        duration: 500,
    }));
...
test('aiTracker.chat tracks chat requests', async t => {
    ...
    const trackedFunction = aiTracker.chat('test-provider', 'test-model', testFunction);
...
test('aiTracker.completion tracks completion requests', async t => {
    ...
    const trackedFunction = aiTracker.completion('test-provider', 'test-model', testFunction);
...
test('aiTracker.embedding tracks embedding requests', async t => {
    ...
    const trackedFunction = aiTracker.embedding('test-provider', 'test-model', testFunction);
...
test('mcpTracker.tool tracks tool execution', async t => {
    ...
    const trackedFunction = mcpTracker.tool('test-server', 'test-tool', testFunction);
...
test('globalRequestTracker can track requests', t => {
    ...
    const requestId = globalRequestTracker.startRequest(createTestRequestMetadata());
...
test('RequestTracker getStats identifies busiest endpoint', t => {
    ...
    for (let i = 0; i < 5; i++) {
        const requestId = tracker.startRequest(createTestRequestMetadata({endpoint: '/api/users'}));
        tracker.completeRequest(requestId, {statusCode: 200});
    }
    
    for (let i = 0; i < 3; i++) {
        const requestId = tracker.startRequest(createTestRequestMetadata({endpoint: '/api/posts'}));
        tracker.completeRequest(requestId, {statusCode: 200});
    }
    
    for (let i = 0; i < 2; i++) {
        const requestId = tracker.startRequest(createTestRequestMetadata({endpoint: '/api/comments'}));
        tracker.completeRequest(requestId, {statusCode: 200});
    }
...
test('RequestTracker getStats identifies most error-prone endpoint', t => {
    ...
    for (let i = 0; i < 8; i++) {
        const requestId = tracker.startRequest(createTestRequestMetadata({endpoint: '/api/good'}));
        tracker.completeRequest(requestId, {statusCode: 200});
    }
    
    // Add mixed requests to another endpoint
    for (let i = 0; i < 5; i++) {
        const requestId = tracker.startRequest(createTestRequestMetadata({endpoint: '/api/bad'}));
        if (i % 2 === 0) {
            tracker.completeRequest(requestId, {statusCode: 200});
        } else {
            tracker.failRequest(requestId, new Error('Test error'));
...
test('RequestTracker getStats tracks requests by type', t => {
    ...
    const httpRequestId = tracker.startRequest(createTestRequestMetadata({type: 'http'}));
    tracker.completeRequest(httpRequestId, {statusCode: 200});
    
    const aiRequestId = tracker.startRequest(createTestRequestMetadata({type: 'ai'}));
    tracker.completeRequest(aiRequestId, {statusCode: 200});
    
    const mcpRequestId = tracker.startRequest(createTestRequestMetadata({type: 'mcp'}));
...
test('RequestTracker preserves custom data', t => {
    ...
    const requestId = tracker.startRequest(createTestRequestMetadata({customData}));
    const completed = tracker.completeRequest(requestId, {statusCode: 200});
