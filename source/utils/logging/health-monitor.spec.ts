...
test('healthChecks.quick returns status', async t => {
	const status = await import('./health-monitor.js').then(m =>
		m.healthChecks.quick(),
	);
	t.true(['healthy', 'degraded', 'unhealthy'].includes(status));
});

test('healthChecks.full returns HealthCheckResult', async t => {
	const result = await import('./health-monitor.js').then(m =>
		m.healthChecks.full(),
	);
	t.truthy(result);
	t.true('status' in result);
...
test('healthChecks.metrics returns SystemMetrics', t => {
	const metrics = import('./health-monitor.js').then(m =>
		m.healthChecks.metrics(),
	);
	t.truthy(metrics);
});

test('healthChecks.ready returns boolean', async t => {
	const ready = await import('./health-monitor.js').then(m =>
		m.healthChecks.ready(),
	);
...
test('healthCheckMiddleware handles health endpoint', async t => {
	const middleware = import('./health-monitor.js').then(m =>
		m.healthCheckMiddleware(),
	);
	t.truthy(middleware);
});
