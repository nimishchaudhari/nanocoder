...
import {checkConfigurationSystem} from './configuration-check.js';
...
test('checkConfigurationSystem returns health check result', t => {
	const config = createMockConfig();
	const result = checkConfigurationSystem(config);

	t.is(result.name, 'configuration-system');
