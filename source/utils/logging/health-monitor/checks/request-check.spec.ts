...
import {checkRequestTracking} from './request-check.js';
...
test('checkRequestTracking returns health check result', t => {
	const config = createMockConfig();
	const result = checkRequestTracking(config);

	t.is(result.name, 'request-tracking');
