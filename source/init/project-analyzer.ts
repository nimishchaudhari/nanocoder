...
export class ProjectAnalyzer {
  ...
  public analyze(): ProjectAnalysis {
    ...
    const codeFiles = scanResult.files.filter(file => 
      this.isCodeFile(file) && !this.isTestFile(file)
    );
    ...
    const importantDirectories = this.getImportantDirectories(scanResult.directories);

    // Get build commands
    const buildCommands = this.frameworkDetector.getBuildCommands();

    // Extract project metadata
    const { projectName, description, repository } = this.extractProjectMetadata();
  ...
  private determineProjectType(
    ...
    const webFrameworks = dependencies.frameworks.filter(f => f.category === 'web');
    const backendFrameworks = dependencies.frameworks.filter(f => f.category === 'backend');
    const mobileFrameworks = dependencies.frameworks.filter(f => f.category === 'mobile');
    const desktopFrameworks = dependencies.frameworks.filter(f => f.category === 'desktop');
  ...
  public getCodingConventions(): string[] {
    ...
    if (analysis.dependencies.testingFrameworks.length > 0) {
      conventions.push(`Write tests using ${analysis.dependencies.testingFrameworks.join(', ')}`);
