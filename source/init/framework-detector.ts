...
export class FrameworkDetector {
  ...
  private checkPackageJson(result: ProjectDependencies): void {
    ...
    try {
      ...
      if (packageJson.scripts) {
        ...
        result.buildInfo.devCommand = packageJson.scripts.dev || packageJson.scripts['dev:server'];
  ...
  private checkCargoToml(result: ProjectDependencies): void {
    ...
    try {
      ...
      if (content.includes('actix-web')) {
        result.frameworks.push({ name: 'Actix Web', category: 'backend', confidence: 'high' });
      }
      if (content.includes('warp')) {
        result.frameworks.push({ name: 'Warp', category: 'backend', confidence: 'high' });
      }
      if (content.includes('rocket')) {
        result.frameworks.push({ name: 'Rocket', category: 'backend', confidence: 'high' });
      }
    } catch (error) {
      // Ignore parsing errors
    }
  ...
  private checkGoMod(result: ProjectDependencies): void {
    ...
    try {
      ...
      if (content.includes('gin-gonic/gin')) {
        result.frameworks.push({ name: 'Gin', category: 'backend', confidence: 'high' });
      }
      if (content.includes('gorilla/mux')) {
        result.frameworks.push({ name: 'Gorilla Mux', category: 'backend', confidence: 'high' });
      }
      if (content.includes('echo')) {
        result.frameworks.push({ name: 'Echo', category: 'backend', confidence: 'high' });
      }
    } catch (error) {
      // Ignore parsing errors
    }
   ...
   */
  private matchFramework(depName: string, version: string): FrameworkInfo | null {
    const pattern = FrameworkDetector.FRAMEWORK_PATTERNS[depName as keyof typeof FrameworkDetector.FRAMEWORK_PATTERNS];
