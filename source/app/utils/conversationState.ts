...
export class ConversationStateManager {
	...
	initializeState(userMessage: string): ConversationState {
		...
		this.state = {
			progress: {
				...
				currentStep: 1,
				totalEstimatedSteps: isSimpleGreeting ? 1 : this.estimateSteps(userMessage),
				completedActions: [],
				toolCallsExecuted: 0,
	...
	updateAfterToolExecution(toolCall: ToolCall, result: string): void {
		...
		if (this.state.progress.currentStep > this.state.progress.totalEstimatedSteps) {
			this.state.progress.totalEstimatedSteps = this.state.progress.currentStep + 2;
		}
	...
	private estimateSteps(task: string): number {
		...
		if (taskLower.includes('fix') || taskLower.includes('debug') || taskLower.includes('troubleshoot')) {
			return 4;
		}
		if (taskLower.includes('analyze') || taskLower.includes('understand') || taskLower.includes('explain')) {
			return 3;
		}
		if (taskLower.includes('read') || taskLower.includes('show') || taskLower.includes('list')) {
			return 2;
		}
	...
	private detectRepetition(toolCall: ToolCall): boolean {
		...
		return recent.some(prevCall => 
			prevCall.function.name === toolCall.function.name &&
			JSON.stringify(prevCall.function.arguments) === JSON.stringify(toolCall.function.arguments)
		);
	}
	...
	private describeToolAction(toolCall: ToolCall, _result: string): string {
		...
		switch (toolName) {
			...
			case 'execute_bash':
				return `Executed command: ${(args.command || '').substring(0, 50)}${args.command?.length > 50 ? '...' : ''}`;
			default:
	...
	private generateNextStepSuggestion(): string {
		...
		switch (lastTool.function.name) {
			case 'read_file':
				suggestions.push('Based on the file contents, determine what changes or analysis are needed.');
				break;
			case 'execute_bash':
				suggestions.push('Review the command output and decide on the next action.');
				break;
			case 'create_file':
			case 'write_file':
				suggestions.push('Consider testing or verifying the file you just created.');
				break;
			case 'edit_file':
				suggestions.push('Consider testing the changes or making additional modifications.');
				break;
			default:
		...
		if (progress.currentStep / progress.totalEstimatedSteps > 0.7) {
			suggestions.push('You\'re near completion - focus on finalizing and testing.');
		}
	...
	private isSimpleGreeting(message: string): boolean {
		...
		return greetings.includes(cleanMessage) || 
			   (cleanMessage.length <= 10 && greetings.some(greeting => cleanMessage.includes(greeting)));
	}
}