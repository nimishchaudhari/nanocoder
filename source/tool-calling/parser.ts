...
export function parseToolCallsFromContent(content: string): ToolCall[] {
	...
	while ((directMatch = directMcpToolCallRegex.exec(content)) !== null) {
		...
		if (
			...
			toolName === 'read_file' ||
			toolName === 'write_file' ||
			fullMatch.includes('<use_mcp_tool>') ||
			fullMatch.includes('<execute_bash>')
		) {
			continue;
		}
	...
	const mcpToolCallRegex =
		/<use_mcp_tool>\s*<server_name>([^<]+)<\/server_name>\s*<tool_name>([^<]+)<\/tool_name>\s*<arguments>([\s\S]*?)<\/arguments>\s*<\/use_mcp_tool>/g;
	let mcpMatch;
