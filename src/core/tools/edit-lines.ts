  ...
  try {
    ...
    if (args.mode === "find_replace") {
      ...
      if (!fileContent.includes(oldText)) {
        return `Error: Text "${oldText}" not found in file`;
      }
    ...
    if (!lineNum) {
      return `Error: line_number is required for mode "${args.mode}"`;
    }
    ...
    if (args.mode === "insert") {
      if (lineNum < 1 || lineNum > lines.length + 1) {
        return `Error: Line number ${lineNum} is out of range for insert (valid range: 1-${lines.length + 1})`;
      }
    } else {
      // For replace, delete, move - line must exist
      if (lineNum < 1 || lineNum > lines.length) {
        return `Error: Line number ${lineNum} is out of range (file has ${lines.length} lines)`;
      }
      
      if (endLine < lineNum || endLine > lines.length) {
        return `Error: End line ${endLine} is out of range or before start line`;
      }
    }
    ...
    switch (args.mode) {
      ...
      case "replace": {
        ...
        description = `Replaced ${rangeDesc} with ${replaceLines.length} line${replaceLines.length > 1 ? 's' : ''}`;
        break;
      }
      ...
      case "move": {
        ...
        if (!targetLine || targetLine < 1 || targetLine > lines.length + 1) {
          return `Error: Target line ${targetLine} is invalid`;
        }
...
const formatter = async (args: any): Promise<string> => {
  ...
  let result = `${toolColor("⚒ edit_lines")}\n`;
  result += `${secondaryColor("Path:")} ${primaryColor(path)}\n`;
  result += `${secondaryColor("Mode:")} ${mode}\n`;
  
  if (mode === "find_replace") {
    result += `${secondaryColor("Operation:")} ${replaceAll ? "Replace all" : "Replace first"}\n`;
  } else {
  ...
  try {
    ...
    switch (mode) {
      case "find_replace": {
        ...
        if (!fileContent.includes(oldText)) {
          ...
          result += `${secondaryColor("Note:")} Operation will fail - text must exist exactly as specified\n`;
          return result;
        }
        ...
        if (occurrences.length > 0) {
          ...
          result += `${successColor(`✓ ${changesToShow.length} change${changesToShow.length > 1 ? 's' : ''} found`)}\n\n`;
          ...
          for (const occurrence of changesToShow) {
            ...
            result += `${secondaryColor(lineNumStr)} ${removedLineColor(`- ${oldLine}`)}\n`;
            result += `${secondaryColor(lineNumStr)} ${addedLineColor(`+ ${newLine}`)}\n`;
            ...
            if (changesToShow.length > 1 && occurrence !== changesToShow[changesToShow.length - 1]) {
              result += "\n";
            }
          }

          if (replaceAll && occurrences.length > changesToShow.length) {
            result += `${secondaryColor(`... and ${occurrences.length - changesToShow.length} more occurrences`)}\n`;
          }
        } else {
          // Handle multiline text
          result += `${errorColor("✗")} Could not locate text in file\n`;
          result += `${secondaryColor("Looking for text starting with:")} ${oldText.split('\n')[0]}\n`;
        }
        break;
      }
      case "insert": {
        if (!lineNumber || lineNumber < 1 || lineNumber > lines.length + 1) {
          result += `${errorColor("✗")} Line number ${lineNumber} is out of range (file has ${lines.length} lines)\n`;
          return result;
        }
        
        const newLines = content.split("\n");
        result += `${successColor(`✓ Inserting ${newLines.length} line${newLines.length > 1 ? 's' : ''}`)}\n\n`;
        ...
        for (let i = 0; i < newLines.length; i++) {
          ...
          try {
            const highlighted = highlight(newLines[i], { language, theme: "default" });
            result += `${secondaryColor(lineNumStr)} ${addedLineColor(`+ ${highlighted}`)}\n`;
          } catch {
            result += `${secondaryColor(lineNumStr)} ${addedLineColor(`+ ${newLines[i]}`)}\n`;
          }
        }
      ...
      case "replace": {
        ...
        result += `${successColor(`✓ Replacing ${linesToRemove} line${linesToRemove > 1 ? 's' : ''} with ${newLines.length} line${newLines.length > 1 ? 's' : ''}`)}\n\n`;
        ...
        for (let i = lineNumber - 1; i < endLine; i++) {
          ...
          try {
            const highlighted = highlight(line, { language, theme: "default" });
            result += `${secondaryColor(lineNumStr)} ${removedLineColor(`- ${highlighted}`)}\n`;
          } catch {
            result += `${secondaryColor(lineNumStr)} ${removedLineColor(`- ${line}`)}\n`;
          }
        }
      ...
      case "delete": {
        const linesToRemove = endLine - lineNumber + 1;
        result += `${successColor(`✓ Deleting ${linesToRemove} line${linesToRemove > 1 ? 's' : ''}`)}\n\n`;
      ...
      case "move": {
        ...
        result += `${successColor(`✓ Moving ${linesToMove} line${linesToMove > 1 ? 's' : ''} to line ${targetLine}`)}\n\n`;
        ...
        for (let i = lineNumber - 1; i < endLine; i++) {
          ...
          const line = lines[i] || "";
          result += `${secondaryColor(lineNumStr)} ${addedLineColor(`+ ${line}`)}\n`;
        }
    ...
    result += `${errorColor("Error:")} ${error instanceof Error ? error.message : String(error)}`;
  }
...
export const editLinesTool: ToolDefinition = {
  ...
  config: {
    ...
    function: {
      name: "edit_lines",
      description: "Edit specific lines in a file (insert, replace, delete, or move lines by line number)",
      parameters: {
        type: "object",
        properties: {
          path: {
          ...
          mode: {
            ...
            description: "The editing operation: 'insert' adds lines, 'replace' replaces line range, 'delete' removes lines, 'move' relocates lines, 'find_replace' finds and replaces text content.",
          },
          line_number: {
            type: "number",
            description: "The starting line number (1-based). Required for insert, replace, delete, move modes.",
          },
          end_line: {
            type: "number",
            description: "The ending line number for range operations (replace, delete, move). If not specified, only affects line_number.",
          },
          content: {
            type: "string",
            description: "The content for insert/replace operations. Can contain multiple lines separated by \\n.",
          },
          target_line: {
            type: "number",
          ...
          old_text: {
            type: "string",
            description: "The text to find and replace (for find_replace mode only).",
          },
          new_text: {
            type: "string",
          ...
          replace_all: {
            type: "boolean",
            description: "Whether to replace all occurrences or just the first one (for find_replace mode only). Default: false.",
            default: false,
          },
        },
        required: ["path", "mode"],
